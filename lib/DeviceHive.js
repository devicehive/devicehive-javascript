...
class DeviceHive{
  constructor({ accessToken, refreshToken, login, password, serverURL }){
    ...
    if (serverURL){
      return api.init(serverURL)
        .then(initializedAPI => this.api = initializedAPI)
        .then(() => 
          new Promise((resolve) => {
            if (accessToken && refreshToken){
              ...
              .then(({ accessToken }) => this.api.setTokens({ accessToken, refreshToken }))
              .then(() => resolve(this));
 ...
 */
  ...
  subscribeCommands(deviceIds, subscriber, commandFilter){
    ...
    if (api.type === `ht`){
      ...
      return this.api.commandsSubscribe(commandFilter, subscriber);
    }
  }
  ...
  subscribeNotifications(deviceIds, subscriber, notificationFilter){
    ...
    if (api.type === `ht`){
      ...
      return this.api.notificationsSubscribe(notificationFilter, subscriber);
    }
  }
